...
export default function AboutPage() {
  return (
    <PageShell
      ...
      actions={
        <>
          <Button variant="outline" asChild className="rounded-full border-[#e3e3e3] bg-white">
    ...
    >
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {highlights.map((item) => (
                <div key={item.label} className="rounded-2xl border border-[#ececec] bg-[#fafafa] p-4">
                  <div className="text-2xl font-semibold text-[#111]">{item.value}</div>
                  <div className="text-xs text-[#767676]">{item.label}</div>
                </div>
              ))}
            </div>
          </CardContent>
        ...
        <div className="space-y-4">
          {values.map((value) => (
            <Card key={value.title} className="rounded-3xl border border-[#e3e3e3] bg-white shadow-sm">
              <CardContent className="p-6">
                <h3 className="text-lg font-semibold text-[#111]">{value.title}</h3>
                <p className="mt-2 text-sm text-[#767676]">{value.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <div className="mt-10 grid gap-6 md:grid-cols-3">
        {mockTeamMembers.map((member) => (
          <Card
            key={member.id}
            className="rounded-3xl border border-[#e3e3e3] bg-white shadow-sm transition-transform hover:-translate-y-1"
          >
            <CardContent className="p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-[#111]">{member.name}</p>
                  <p className="text-xs text-[#767676]">{member.role}</p>
                </div>
              </div>
              <p className="mt-3 text-sm text-[#767676]">{member.bio}</p>
              <p className="mt-3 text-xs text-[#767676]">{member.location}</p>
            </CardContent>
          </Card>
        ))}
      </div>
