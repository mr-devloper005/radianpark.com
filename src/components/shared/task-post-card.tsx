...
export function TaskPostCard({
  ...
  if (experience.key === "tynewebdesign") {
    if (variant === "image") {
      return (
        <Link
          href={href}
          className="group mb-6 block break-inside-avoid overflow-hidden rounded-[2.25rem] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.95)_0%,rgba(248,252,255,0.92)_100%)] p-3 shadow-[0_24px_70px_rgba(14,116,144,0.12)] transition hover:-translate-y-1 hover:shadow-[0_30px_90px_rgba(14,116,144,0.18)]"
        >
          <div className="relative overflow-hidden rounded-[1.8rem] bg-slate-100">
            <div className="absolute left-4 top-4 z-10 rounded-full bg-sky-950/90 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-white shadow-lg">
              {category}
            </div>
            ...
            <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-slate-950/65 via-slate-950/20 to-transparent" />
          </div>

          <div className="-mt-8 relative z-10 mx-3 rounded-[1.6rem] border border-white/80 bg-white/92 p-5 shadow-[0_16px_40px_rgba(15,23,42,0.08)] backdrop-blur-xl">
            <div className="flex items-center justify-between gap-3">
              <span className="text-[11px] font-semibold uppercase tracking-[0.24em] text-sky-800">
                Visual post
              </span>
              <ArrowUpRight className="h-4 w-4 text-slate-400 transition group-hover:text-sky-900" />
            </div>
            <h3 className="mt-3 text-[1.45rem] font-semibold leading-tight tracking-[-0.03em] text-slate-950">
              {post.title}
            </h3>
  ...
  if (experience.key === "radianpark") {
    return (
      <Link href={href} className="group flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-zinc-200 bg-white shadow-[0_8px_30px_rgba(24,24,27,0.06)] transition hover:shadow-[0_12px_40px_rgba(24,24,27,0.1)]">
        <div className="relative overflow-hidden">
          <div className="absolute left-4 top-4 z-10 rounded-full bg-zinc-950 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-white">
            {category}
          </div>
        ...
        <div className="flex flex-1 flex-col p-5">
          <h3 className="text-lg font-semibold leading-snug text-zinc-950 line-clamp-2">{post.title}</h3>
          <p className="mt-2 text-sm leading-6 text-zinc-500 line-clamp-3">{excerpt}</p>
          <div className="mt-auto pt-4 flex items-center gap-2 text-xs text-zinc-400">
            <Tag className="h-3.5 w-3.5" />
            <span>{category}</span>
          </div>
        </div>
